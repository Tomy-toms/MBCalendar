...
const token = localStorage.getItem('authToken');
                ...
                if (livreur) {
                    ...
                    for (let day = 1; day <= 5; day++) {
                        ...
                        cell.addEventListener('drop', e => {
                            ...
                            handleTaskDrop(taskId, parseInt(day), livreur.id);
                        });
        ...
        //fonction pour créer un élément de tâche -- ORIGINAL --
        function createTaskElement(task) {
            ...
            const chantierText = chantier ? `<div>${chantier.name}<br><small>${chantier.address}</small></div>` : '';
            
            taskElement.innerHTML = `<strong>${task.description}</strong><br>${chantierText}<br><em>${task.notes || ''}</em>`;
            taskElement.dataset.taskId = task.id;
            
            // Gestion du clic pour sélectionner la tâche
